...
int main(int argc, char **argv) {
  ...
  wb_camera_enable(camera,CONTROL_STEP);
  wb_camera_recognition_enable(camera,CONTROL_STEP);
  // camera device
  if (has_camera) {
    //camera = wb_robot_get_device("camera");
  ...
  WbNodeRef robot_node = wb_supervisor_node_get_from_def("traffic_light_1");
  WbFieldRef light = wb_supervisor_node_get_field(robot_node, "state");
      

  // main loop
  while (wbu_driver_step() != -1) {
  
   //const double *x=wb_supervisor_field_get_sf_color("traffic_light_1");
  // printf("%f",x[0]);
  /*WbFieldRef light = wb_supervisor_node_get_field(robot_node, "state");
  const char *x=wb_supervisor_field_get_sf_string(light);
      printf("%s",x);*/
    // get user input
    check_keyboard();
    static int i = 0;
    ...
    const char *x=wb_supervisor_field_get_sf_string(light);
     if(strcmp(objects[k].model,"traffic light")==0){
      //printf("%s\n", objects[k].model);
      //const char *x=wb_supervisor_field_get_sf_string(light);
      printf("traffic signal is  %s\n",x);
      set_speed(0);
      if(strcmp(x,"red")==0)
      {
         set_speed(0);
     ...
     /* WbNodeRef robot_node = wb_supervisor_node_get_from_def("traffic_light_1");
      WbFieldRef light = wb_supervisor_node_get_field(robot_node, "state");
      */
      //wb_camera_recognition_get_objects(GenericTrafficLight).state;
       //set_speed(40);
      }
      //if(strcmp(objects[k].model,"pedestrian crossing")==0){
     // set_speed(0);
     // }
